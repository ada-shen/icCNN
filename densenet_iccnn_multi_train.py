...
log_path = './icCNN/run/densenet/' # for model
save_path = './icCNN/run/basic_fmap/densenet/'  # for get_feature
acc_path = './icCNN/run/basic_fmap/densenet/acc/'
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=NUM_CLASSES, memory_efficient=MEMORY_EFFICIENT):
        ...
        self.smg = SMGBlock(channel_size = CHANNEL_NUM ,f_map_size=F_MAP_SIZE)  # 14*14 feature map #CHANNEL_SIZE
...
def net_train():
    ...
    # Train the model
    save_similatiry_loss = [];save_gt=[];save_class_loss= [];save_total_loss = [];
    ...
    for epoch in range(EPOCH+1):
        if epoch % T==0 and epoch < STOP_CLUSTERING:
            ...
            scheduler.step()
            net.train()
            total_loss = 0.0;similarity_loss = 0.0;class_loss = 0.0
